...
## 💡 コラム: 「ほとんどすべての二分探索は壊れている」

2006年、Java の設計に関わった著名エンジニアのジョシュア・ブロックが衝撃的な告白をしました。彼が書いた Java 標準ライブラリの二分探索に、**約9年間誰にも気づかれなかったバグ**があったのです。
...
一見完璧です。しかし配列が巨大(約10億要素以上)になると `low + high` が int の上限を超えてオーバーフローし、負の値になって壊れます。修正は `low + (high - low) / 2` と書くこと。

さらに驚くべきことに、ある調査では、プロのプログラマーに二分探索を書かせると**約9割が何らかのバグを入れる**という結果が出ています(境界条件のずれ、無限ループなど)。「アイデアが単純」と「実装が簡単」は全くの別物 — 二分探索は、境界条件に対する謙虚さを教えてくれる最良の教材です。
