...
## 💡 コラム: なぜ「終端(ターミナル)」と呼ぶのか

「ターミナル」は元々、比喩ではなく物理的な「配線の終端」でした。1960〜70年代、コンピュータは部屋一つ分の大きさで大学に1台。利用者はキーボードと画面(または印字用紙!)だけの装置を配線の端に繋いで共同利用しました。この端末装置がターミナルです。さらに昔はタイプライター式の「テレタイプ(teletype)」が使われ、その略 **tty** は今も Linux のデバイス名(`/dev/tty`)に残っています。

つまり、いまあなたが使っているのは「ターミナル・エミュレータ」— 紙とタイプライターの時代の端末の**ふりをする**ソフトウェアです。Enter を押すまで1行ずつ送信するスタイルも、その名残です。

コマンドの短さ(`ls`, `cp`, `mv`)も歴史の産物です。当時は通信が遅く、1文字打つのも重かったので、コマンドは極限まで短くなりました。50年前の制約が生んだ道具が、今も最速の操作手段であり続けているのは面白いことです。
