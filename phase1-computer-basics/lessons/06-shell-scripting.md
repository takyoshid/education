...
## 💡 コラム: 変数が空だと何が起きるか — Steam 事件

2015年、ゲームプラットフォーム Steam の Linux 版で、一部ユーザーのホームディレクトリのファイルがごっそり消える事件が起きました。原因はインストールスクリプト内のこの1行です。

```bash
rm -rf "$STEAMROOT/"*
```

ある条件で変数 `STEAMROOT` が**空文字列**になり、コマンドは `rm -rf "/"*` — つまり「ルート直下から消せるものを全部消せ」に化けたのです。

シェルスクリプトは「変数が空でも黙って実行してしまう」言語です。だからプロは防御的に書きます:

- `set -u`(未定義変数の使用をエラーにする)をスクリプト冒頭に書く
- `rm` のような破壊的コマンドの前では、変数の中身を疑う

たった1行のスクリプトが世界中のファイルを消せる — この怖さを知っていることが、シェルを扱う者の免許証です。
