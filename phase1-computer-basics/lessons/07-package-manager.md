...
2016年3月、世界中の JavaScript プロジェクトのビルドが突然失敗し始めました。React も Babel も巻き込んだ大混乱の原因は、`left-pad` という**たった11行のパッケージ**(文字列の左側に空白を詰めるだけ)が、作者との揉め事の末にリポジトリから削除されたことでした。

無数の有名パッケージが、間接的にこの小さなパッケージに依存していたのです。誰もそれを意識していませんでした — 消えるまでは。

この事件は「現代のソフトウェアは、見ず知らずの誰かが善意で公開した無数の部品の上に建っている」という事実を世界に突きつけました。パッケージマネージャは魔法の`install`ボタンではなく、**他人のコードを自分のシステムに招き入れる門**です。門番として、何をインストールしているのか意識する習慣を持ちましょう。
