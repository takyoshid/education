...
セマンティック HTML の価値が最も切実に現れるのは、**画面を見ないユーザー**に対してです。視覚障害のあるユーザーはスクリーンリーダー(読み上げソフト)で Web を使います。ページ内の見出しだけを順にジャンプして「目次」のように全体を把握し、「ナビゲーションを飛ばす」「メインコンテンツへ移動」といった操作を駆使します。

このときページが `<div>` の山だと、スクリーンリーダーには構造が全く見えません。のっぺりした文字の羅列を先頭から聞くしかない。一方 `<h1>`、`<nav>`、`<main>` が正しく使われていれば、「見出しレベル1、ニュース一覧」「ナビゲーション、リンク5個」と、ページの地図が音声で伝わります。

新聞に例えるなら、見出しもリード文もなく全記事が同じ文字サイズでベタ組みされた紙面を想像してください — 読めたものではありません。そして検索エンジンのクローラーも、実は同じ「目の見えない読者」です。セマンティクスは思いやりであると同時に、SEO という実利でもあります。
