...
## 💡 コラム: 鉄の箱が世界貿易を変え、Docker がデプロイを変えた

Docker のロゴがコンテナを積んだクジラである理由は、名前の由来そのもの — **海運のコンテナ革命**です。

1956年以前、船の積み荷は樽、木箱、袋がバラバラに積まれ、荷役に数週間かかることもありました。マルコム・マクリーンという運送業者が導入した「**規格化された鉄の箱**」がすべてを変えます。中身が何であれ、箱の規格さえ同じなら、世界中のどのクレーン・船・トラックでも同じように扱える。荷役は数時間になり、輸送コストは劇的に下がり、世界貿易の構造そのものが変わりました。

Docker はこれをソフトウェアで再現しました。**アプリも依存ライブラリも設定も「規格化された箱」に詰めれば、開発者のノート PC でも本番サーバーでも同じように動く。** Phase 0 の「私のマシンでは動くんですけど」問題への、構造的な回答です。

ちなみに Docker の初披露は2013年、あるカンファレンスの**5分間のライトニングトーク**でした。5分のデモが業界を変えた — 歴史はときどき、あっけない場所から始まります。
