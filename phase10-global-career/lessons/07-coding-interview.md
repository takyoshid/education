...
あなたがこの逸話から持ち帰るべき結論は、冷静なものです: **コーディング面接は、実務能力の完全な代理ではない「別競技」である。** 理不尽に感じるかもしれませんが、別競技だと割り切れば見え方が変わります — ルールが公開されていて、出題パターンが有限で、**対策した分だけ確実に点が上がる**競技です。Phase 3 の知識を面接の「型」(声に出して考える、まず総当たり、計算量を言う)に載せる訓練 — このレッスンでやるのは、その競技準備です。
