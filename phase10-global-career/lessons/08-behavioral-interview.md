...
## 💡 コラム: 「チームワークが得意です」が0点である理由

Amazon の採用面接には「**バーレイザー(Bar Raiser)**」という特別な面接官が同席します。配属予定チームと利害関係のない社員で、その役割はただ一つ、「採用の基準(bar)を下げさせない」こと。彼らが徹底的に使うのが行動面接です。

行動面接の背後には、採用研究で繰り返し支持されてきた原則があります — 「**過去の行動は、未来の行動の最良の予測材料である**」。だから「あなたはリーダーシップがありますか?」(誰でも Yes と言える)ではなく、「**リーダーシップを発揮した具体的な場面を、一つ話してください**」(実体験がないと語れない)と聞くのです。

これが「チームワークが得意です」が0点である理由です。それは主張であって証拠ではありません。満点の答えは物語です — 「リリース前夜に障害が起きて(**状況**)、私はログ解析の担当で(**課題**)、切り分けのためにこうして(**行動**)、原因を特定してリリースは半日遅れで済んだ(**結果**)」。これが STAR 形式の正体で、つまり行動面接の対策とは、英語力より先に、**自分の経験の棚卸しと物語化**なのです。
