...
## 💡 コラム: ログは飛行機のブラックボックスである

なぜ運航に不要なブラックボックスを、すべての旅客機が積んでいるのか。**墜落は「起きてから」しか調べられない**からです。事故調査官が頼れるのは、その瞬間に何が記録されていたかだけ。「あの時のデータを記録しておけば」という後悔は、事故の後では取り返しがつきません。

サーバーのログも全く同じです。本番の障害は再現できないことが多く、調査の材料は**障害が起きる前に仕込んでおいたログがすべて**。障害対応の定番の後悔が「ここにログを入れておけば一発だったのに」です。だからログ設計は「異常が起きたとき、未来の自分は何を知りたいか?」という想像力の仕事なのです(ちなみに Phase 10 では、これの発展形として「計器(メトリクス)」との使い分けを学びます)。

設定の外部化にも一言で言える原則があります。有名な「Twelve-Factor App」の教え — 「**コードはすべての環境で同一に、環境ごとの違いは環境変数に**」。コードに `if 本番なら` が現れ始めたら、それは設定がコードに漏れ出しているサインです。
