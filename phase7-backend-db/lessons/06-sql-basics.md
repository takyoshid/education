...
## 💡 コラム: SQL はタクシーの乗り方である

SQL が他の多くの言語と根本的に違うのは、「**どうやって**取ってくるか」を書かない点です。書くのは「**何が**欲しいか」だけ。これを宣言型言語と呼びます。

タクシーに例えると分かりやすいです。「駅までお願いします」と目的地だけ告げるのが SQL(宣言型)。「200m 先を右、次の信号を左、そこから直進…」と運転手に逐一指示するのが Python のような手続き型です。

**経路選び(実行計画)はドライバー(クエリオプティマイザ)の仕事**です。オプティマイザはテーブルの統計情報を見て、「インデックスを使うか」「どの順でテーブルを結合するか」を毎回計算し、最適なルートを選びます。渋滞情報を知り尽くしたベテラン運転手です。

だから SQL 上達の道筋も独特です。まずは「欲しい結果を正確に記述する」ことに集中すればいい。そして中級者になったら、`EXPLAIN` コマンドで「運転手が実際に選んだルート」を覗く方法を学ぶ — それが Lesson 07 以降の世界です。
