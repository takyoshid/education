...
## 💡 コラム: 「それを REST と呼ばないでくれ」— 提唱者の嘆き

「REST」という言葉は、ロイ・フィールディングが2000年に書いた博士論文が出典です。彼は HTTP/1.1 仕様の主著者の一人でもあり、論文は「Web がなぜこれほどスケールしたのか」を建築様式(アーキテクチャスタイル)として抽出したものでした。

面白いのはその後です。世の中に「REST API」が溢れると、フィールディング本人がブログで**「それは私の定義する REST ではない」と苦言を呈した**ことがあります(彼の定義では、ハイパーメディアによる状態遷移などの厳格な条件があります)。学術用語が普及の過程で「HTTP をそれっぽく使った API」全般を指すようになった、珍しい例です。

実務での構え方はこうで十分です: 論文の厳密な定義を暗記する必要はなく、**「URL は名詞(リソース)、HTTP メソッドは動詞、ステータスコードは結果」という HTTP の意味論に素直に従う** — このゆるい共通理解こそが、初対面の開発者同士でも API の挙動を予測し合える価値の源泉です。
