...
## 💡 コラム: インデックス1本で1000倍 — 辞書に索引がある理由

インデックスの本質は、辞書の索引と同じです。「り」で始まる単語を探すのに、1ページ目から順にめくる人はいません。索引(五十音順に整列された案内)を見て一発でページに飛ぶ — これが O(n) のフルスキャンと O(log n) のインデックス検索の違いです。

実務では「**遅かった画面に INDEX を1本張ったら、数分が数十ミリ秒になった**」という類の逸話が世界中に無数にあります。桁が3つ4つ変わる改善が、たった1行の `CREATE INDEX` で起きる。Phase 4 で学んだ計算量の知識が、最も劇的な形で現金化される場所がデータベースです。

ではなぜ全列に張らないのか? 索引付きの辞書に単語を**追加する**場面を想像してください。本文に書き足すだけでなく、索引も正しい位置に挿入し直す必要があります。インデックスは**読みを速くする代わりに、書き込みを遅くし、容量を食う**トレードオフの道具です。「よく検索される列に、必要なぶんだけ」— これがプロの張り方です。
