...
## 💡 コラム: 「ORM はコンピュータサイエンスのベトナム戦争」

2006年、エンジニアのテッド・ニューワードが書いたエッセイの一節は、今も引用され続けています — 「**ORM はコンピュータサイエンスのベトナム戦争である**」。最初は快進撃(簡単な CRUD が一瞬で書ける)、しかし深入りするほど泥沼(複雑なクエリ、性能問題、抽象化の漏れ)にはまっていく、という辛辣な比喩です。

泥沼の代表が **N+1 問題**です。「ユーザー100人と、各自の投稿を表示する」つもりで書いた数行の ORM コードが、裏では**ユーザー一覧に1回 + 各ユーザーの投稿取得に100回 = 101回**のクエリを発行している。開発機(データ10件)では気づかず、本番(データ10万件)で爆発する、典型的な時限爆弾です。

ORM は強力な道具で、使わない選択肢は現実的ではありません。プロと初心者を分けるのは一つの習慣だけです — **「この ORM コードは、裏でどんな SQL を何回発行しているか?」を確認する**こと(SQLAlchemy なら `echo=True`)。魔法の裏側を覗ける人だけが、魔法を安全に使えます。
