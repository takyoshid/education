...
## 💡 コラム: クジラのエラー画面が名物だった頃

スケールアップとスケールアウトの違いは、運送業で例えるのが定番です。**スケールアップ = より大きなトラックに買い替える**(シンプルだが、トラックの大きさには物理的・価格的な上限がある)。**スケールアウト = トラックの台数を増やす**(上限は緩いが、「どの荷物をどのトラックに積むか」の采配役 = ロードバランサが必要になる)。

スケーラビリティの苦闘の実例として歴史に残るのが、初期の Twitter です。2000年代後半、急成長に耐えられずサービスは頻繁にダウンし、そのたびに表示される**クジラを鳥たちが吊り上げるエラー画面「Fail Whale」**は、皮肉にも世界的な人気キャラクターになりました。Twitter はその後、数年がかりでアーキテクチャを抜本的に作り替え、クジラは引退しました。

ここに視点の転換があります。スケーラビリティの問題は、**成功したサービスだけが直面できる、贅沢で切実な問題**だということです。だから設計の鉄則は「最初から Twitter 規模に備える」ではなく、「**計測し、ボトルネックを特定し、必要になった箇所から拡張できる構造にしておく**」。早すぎる最適化は、来ないかもしれない渋滞のために高速道路を10車線で作るようなものです。
