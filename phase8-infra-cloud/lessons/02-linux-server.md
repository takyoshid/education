...
2021年4月、NASA のヘリコプター「Ingenuity」が火星で飛行に成功しました — **人類初の、地球以外の惑星での動力飛行**です。このヘリコプターの頭脳で動いていた OS は、特注の宇宙専用システムではなく **Linux** でした。市販に近い部品とオープンソースソフトウェアの組み合わせが、火星の空を飛んだのです。

地球上でも Linux の支配は圧倒的です。世界のスーパーコンピュータ TOP500 は **100% Linux**。Web サーバーの大多数、Android スマホの中身、銀行の基幹系、そしてあなたがこれから触るクラウドのサーバーも、ほぼ確実に Linux です。

つまり Phase 1 で学んだ CLI と Linux の知識は、「古臭い黒い画面の教養」ではなく、**地球の(そして地球外の)計算インフラを直接操作する免許**です。このレッスンでサーバーに SSH で入るとき、あなたは火星のヘリコプターと同じ系譜の OS に話しかけています。
