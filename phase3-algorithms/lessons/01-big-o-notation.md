...
2007年、Google 本社でのイベントで、CEO のエリック・シュミットが当時大統領候補だったオバマ上院議員に冗談で技術面接の質問をしました。「100万個の32ビット整数をソートする最も効率的な方法は?」

オバマの返答 — 「**バブルソートは、違うだろうね(I think the bubble sort would be the wrong way to go)**」。会場は大爆笑。仕込みだったにせよ、大統領候補の口から O(n²) アルゴリズムへのダメ出しが出た瞬間でした。

実は、人間は日常的に計算量を最適化しています。紙の辞書で単語を探すとき、1ページ目から順にめくる人はいません。真ん中を開いて、前か後ろかを判断して半分に絞る — これは O(n) ではなく O(log n) の二分探索です。Big-O は難解な数学ではなく、**「そのやり方、データが100万件になっても大丈夫?」を一言で言うための共通言語**なのです。
