...
## 💡 コラム: 面接の頻出問題なのに、実務で見かけない理由

連結リストは宝探しゲームに似ています。各地点にあるのは宝の一部と「次の地点へのヒント」だけ。全体地図(配列のような一覧性)はありません。その代わり、途中に新しい地点を差し込むのは**紙切れ1枚を書き換えるだけ** — 配列のように「後ろ全員を1マスずつずらす」必要がないのです。

ところで、連結リストには不思議な立ち位置があります。**コーディング面接では超頻出なのに、実務のアプリケーションコードで生で書くことはほぼない**のです。Python ならリスト(内部は動的配列)で大抵済んでしまいます。

では無駄な知識か? いいえ。連結リストは OS のメモリ管理(フリーリスト)、LRU キャッシュ、Git のコミット履歴(各コミットが親を指す)など、**あなたが毎日使う道具の内部**で生きています。そして面接で問われ続ける理由は「ポインタ(参照)を正確に操作できるか」を測る最小の題材だから。参照の付け替えを紙に描いて追える人は、どんな複雑なデータ構造にも立ち向かえます。
