...
## 💡 コラム: Google 検索「recursion」の公式ジョーク

Google で「recursion(再帰)」と検索してみてください。検索結果の上部に「**もしかして: recursion**」と表示されます。クリックしても同じページ、また「もしかして: recursion」— Google エンジニアが仕込んだ、再帰の完璧な実演です(ベースケースがないので、あなたが飽きるまで続きます)。

プログラマー文化には再帰ジョークが豊富です。「再帰を理解するには、まず再帰を理解する必要がある」。GNU の名前は「**G**NU's **N**ot **U**nix」の略で、展開すると「(GNU's Not Unix)'s Not Unix」と無限に続く再帰的頭字語です。

イメージとしてはマトリョーシカ人形が最適です。開けると一回り小さい同じ形の人形が出てくる(**同じ問題のより小さい版**)。そして一番小さい人形は開かない(**ベースケース**)。ベースケースを忘れた再帰は、合わせ鏡の間に立ったときの無限の廊下 — 美しいですが、プログラムは stack overflow で死にます。ジョークで笑えるようになったら、再帰は理解できています。
