...
## 💡 コラム: 80年前の数学の本が、いまも面接対策の最強書である理由

1945年、数学者ジョージ・ポリアが『いかにして問題をとくか』という本を書きました。数学教育の本ですが、80年後の今もプログラマーの問題解決とコーディング面接の土台であり続けています。ポリアの問いかけリストは、そのままアルゴリズム問題に使えます:

- **未知のものは何か? 与えられているものは何か?**(入力と出力を正確に把握する)
- **似た問題を解いたことはないか?**(パターンの照合 — このレッスンで学んだこと)
- **問題を言い換えられないか?**(「重複を探す」は「見たものを覚えておく」= セットの出番)
- **もっと簡単な問題なら解けるか?**(まず総当たりで解いてから改善する)

パターン学習の本質は暗記ではなく「**既視感のインデックス作り**」です。将棋の棋士が定跡を学ぶのは、盤面を見た瞬間に「この形は知っている」と絞り込むため。あなたが演習を重ねる意味も同じで、初見の問題を「どこかで会った問題」に変換する索引を、頭の中に育てているのです。
