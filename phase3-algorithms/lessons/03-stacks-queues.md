...
## 💡 コラム: 世界はスタックでできている — サイト名の由来

世界最大のプログラミング Q&A サイト「Stack Overflow」の名前は、エラーメッセージが由来です。関数が関数を呼ぶたびに「呼び出し元に戻るための情報」がコールスタックに積まれ、再帰しすぎるとスタックが溢れる — それが stack overflow エラーです。プログラマーなら誰もが見たことのあるエラー名を、そのままサイト名にしたわけです。

例えは食べ物と行列で:

- **スタック** = パンケーキの山。上に積んで、上から食べる(LIFO)。ブラウザの「戻る」ボタン、Ctrl+Z(元に戻す)、関数呼び出し — 「直近のものから巻き戻す」場面はすべてスタックです
- **キュー** = 人気ラーメン店の行列。先に並んだ人が先に食べる(FIFO)。印刷ジョブ、メッセージ配送、幅優先探索 — 「順番を守る」場面はすべてキューです

新しい問題に出会ったら「これは巻き戻し型(スタック)か、順番待ち型(キュー)か?」と問うだけで、設計の半分が決まります。
