...
## 💡 コラム: なぜ 0 から数えるのか — ダイクストラのメモ

「リストの先頭が `[1]` じゃなくて `[0]` なのは分かりにくい」— 誰もが一度は思う疑問に、計算機科学の巨人ダイクストラが1982年に短い手書きメモで答えています。要点は: 範囲を「0以上 N 未満」(`0 <= i < N`)で表すと、**要素数がそのまま N になり、範囲の連結時に境界が重ならず、空の範囲も自然に表せる**。0始まりは慣習ではなく、バグを減らす合理的な設計なのです。`range(5)` が 0〜4 を返すのも同じ思想です。

データ構造の使い分けは施設で例えられます:

- **リスト** = 番号付きロッカー(順番に並んでいて、番号で取り出す)
- **辞書** = クロークの引換札(札 = キーを渡せば、中身が返ってくる。順番は本質ではない)
- **セット** = 会員名簿(「いるか、いないか」だけに答える。同姓同名の重複登録はできない)

「順番が大事ならリスト、名前で引くなら辞書、所属判定ならセット」— この一文が使い分けのすべてです。
