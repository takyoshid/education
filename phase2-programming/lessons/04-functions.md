...
関数の最も分かりやすい例えは自動販売機です。お金とボタンの選択(**引数**)を入れると、飲み物(**戻り値**)が出てくる。そして重要なのは、**中の仕組みを知らなくても使える**ことです。内部で冷却装置がどう動いているかを知らなくても、ジュースは買えます。

これが「抽象化」— プログラミングにおける最強の武器です。`len("hello")` と書くとき、あなたは len の内部実装を知りません。知る必要がないのです。

良い関数の条件もここから導けます: **名前と引数を見ただけで、何が起きるか予想できること**。「ボタンにコーラと書いてあるのにお茶が出る」自販機は最悪です。同様に、`calculate_total()` という名前でデータの削除もする関数は、チームの信頼を破壊します。UNIX 哲学の言葉を借りれば「一つのことを、うまくやる」— これが関数設計の北極星です。
