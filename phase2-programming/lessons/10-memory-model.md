...
「あれ、正しく動いているのでは?」と思ったなら、それがこのバグの一番怖いところです。**たまたま答えが合っているだけ**で、理由はまったく正しくありません。

`for` はリストを内部のインデックスで走査します。要素を削除するとその位置以降が前へ詰まるため、**インデックスだけが進んで要素が飛ばされます**。

```
i=0: numbers=[1,2,3,4,5,6]  → numbers[0]=1  奇数、何もしない
i=1: numbers=[1,2,3,4,5,6]  → numbers[1]=2  偶数、削除 → [1,3,4,5,6]
i=2: numbers=[1,3,4,5,6]    → numbers[2]=4  偶数、削除 → [1,3,5,6]
                               ↑ 3 は飛ばされた(見ていない)
i=3: numbers=[1,3,5,6]      → numbers[3]=6  偶数、削除 → [1,3,5]
i=4: numbers=[1,3,5]        → 長さ 3 を超えたのでループ終了
                               ↑ 5 は検査されていない
```

3 と 5 は**一度も検査されないまま**残り、たまたま奇数だったので結果が正しく見えているだけです。要素を `[1, 2, 4, 6]` に変えれば、`4` が飛ばされて結果は `[1, 4]` になり、バグが表面化します。
...
for n in numbers:
    if n % 2 == 0:
        numbers.remove(n)
print(numbers)    # [1, 4]  ← 偶数の 4 が残ってしまった

# 修正 1: 新しいリストを作る(推奨)
numbers = [n for n in numbers if n % 2 != 0]

# 修正 2: コピーをループする(元のリストを変更したい場合)
for n in numbers[:]:      # numbers[:] はコピー
    if n % 2 == 0:
        numbers.remove(n)
```

**教訓**: 「動いたから正しい」ではありません。**なぜ動いたかを説明できて初めて正しい**のです。
...
5. 次の 2 つの関数は、どちらもミュータブルなデフォルト引数 `[]` を使っています。
   **片方だけにバグがあります。** どちらがバグで、どちらが無害か、理由とともに答えてください。
   ...
   # B
   def collect_negatives(n, found=[]):
       if n < 0:
           found.append(n)
       return found
   ```

   (ヒント: 「デフォルト引数のリストは全呼び出しで共有される」ことは両方に当てはまります。
   問題になるのは、それを**変更したとき**だけです。)

6. 上の設問で「無害」と判断したほうも、`ruff` や `flake8` では警告(B006)が出ます。
   バグではないのに警告する理由は何だと思いますか。この警告に従うべきでしょうか。
