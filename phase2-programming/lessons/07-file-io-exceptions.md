...
「許可を求めるより、許しを請うほうが簡単だ」— コンピュータ黎明期の伝説的プログラマー、グレース・ホッパー海軍准将の有名な言葉です(彼女は Lesson 11 の「バグ」の逸話にも登場します)。

実はこの言葉、Python の例外処理の文化そのものです。Python では **EAFP**(Easier to Ask Forgiveness than Permission)というスタイルが好まれます: 事前チェックを重ねるのではなく、**まず実行してみて、ダメだったら `except` で対処する**。
...
ファイル操作は EAFP が特に合理的な領域です。「存在チェックしてから開く」方式では、チェックと open の間の一瞬にファイルが消される可能性が理論上残ります。最初から `try` で包めば、その隙間はありません。
