...
**「黙って壊れる」より「エラーで止まる」ほうが常に良い**です。Phase 12 のコラムで扱う 2003年の大停電も、根は同じ性質でした。
...
| 同じリクエストか | キーをソートした JSON(演習 12 / Phase 12 の冪等性) |

計算量は、単語数 n・最大単語長 k に対して **O(n × k log k)**(各単語のソート)です。文字が英小文字だけなら、26 要素のカウントをキーにして O(n × k) にできます。Phase 3 でこの種の最適化を扱います。
