...
`__all__` は「これが公開 API です」という意思表示です。書かれていない名前は内部実装であり、いつ変わってもおかしくない、というメッセージになります。Phase 9 の「カプセル化」と同じ発想が、パッケージの粒度で現れたものです。
...
`misses=31` は「実際に計算した回数」(n=0..30 の 31 個)、`hits=28` は「キャッシュから返した回数」です。Phase 4 の動的計画法(メモ化)の、最も手軽な形です。
