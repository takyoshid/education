...
## 💡 コラム: 十徳ナイフより、切れ味のいい包丁を

UNIX 哲学の第一条は「**一つのことを、うまくやれ(Do One Thing and Do It Well)**」です。Phase 1 で見たパイプの世界を思い出してください。`sort` は並べ替えしかせず、`uniq` は重複除去しかしない。単機能だからこそ、無限の組み合わせが生まれました。

関数とクラスの設計も同じです。プロの料理人の道具は十徳ナイフではなく、**用途ごとに研ぎ澄まされた包丁の並んだ引き出し**です。十徳ナイフ(何でもやる関数)は一見便利ですが、どの用途でも二流で、1つの刃を研ぐと他の刃に影響が出ます。

実務で使える危険信号を2つ:

- **名前に「and」が入る**(`validate_and_save()`)— 2つの仕事をしている自白です。分割しましょう
- **関数の説明に「ただし」「場合によっては」が必要** — 呼び出す側が内部を知らないと安全に使えないサインです

「この関数の仕事を一文で言えるか?」— この問いだけで、設計の質は大きく変わります。
