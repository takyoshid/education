...
## 💡 コラム: ステータスコード 418「私はティーポットです」

HTTP ステータスコードには、公式の冗談が1つ紛れています。**418 I'm a teapot(私はティーポットです)** — 「ティーポットにコーヒーを淹れろと要求した場合に返すべきエラー」です。1998年のエイプリルフールに発行された「コーヒーポット制御プロトコル」という冗談 RFC が出典です。

2017年、あるフレームワークの開発者が「冗談コードは削除しよう」と提案したところ、世界中の開発者が **save418.com** を立ち上げて反対運動を展開。結局 418 は「使用しないコードとして正式に予約」され、守られました。Google など多くのサービスは、今も遊び心で 418 を実装しています。

この騒動が教えてくれるのは、プロトコルが「無味乾燥な仕様」ではなく**人間の文化**だということです。そして実利的な教訓も一つ: ステータスコードは分類体系(2xx 成功 / 4xx あなたのミス / 5xx こちらのミス)さえ頭に入れれば、初見のコードでも慌てず読める、ということです。ティーポットでさえ 4xx(クライアントの要求が悪い)に分類されています。
