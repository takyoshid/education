...
サーバーのログも全く同じです。本番の障害は再現できないことが多く、調査の材料は**障害が起きる前に仕込んでおいたログがすべて**。障害対応の定番の後悔が「ここにログを入れておけば一発だったのに」です。だからログ設計は「異常が起きたとき、未来の自分は何を知りたいか?」という想像力の仕事なのです(ちなみに Phase 8 では、これの発展形として「計器(メトリクス)」との使い分けを学びます)。
