...
イベントバブリングは、学校の報告経路に例えると一発で分かります。教室でトラブルが起きると、報告は**担任 → 学年主任 → 教頭 → 校長**へと階層を上がっていきます。これがバブリング — ボタンで起きたクリックイベントが、親要素、そのまた親、最終的に document まで「泡のように」浮かび上がっていく仕組みです。

ここで重要なのは、校長(親要素)は「**どの教室で誰が起こした件か**」(`event.target`)を知れることです。

この性質を活かすのが**イベントデリゲーション**という頻出テクニックです。リストの項目1000個それぞれに聞き耳(リスナー)を立てる代わりに、校長室に1つだけリスナーを置き、「報告が上がってきたら target を見て対応する」。項目が後から増えても対応不要、メモリも節約 — 「上に上がってくる」という仕組みの理解が、そのまま実務の武器になります。
