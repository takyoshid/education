...
## 💡 コラム: 依存性逆転はコンセントの規格である

SOLID の中で最も抽象的で挫折率の高い「依存性逆転の原則(DIP)」は、壁のコンセントで考えると一気に具体化します。

ドライヤーは「発電所」に依存していません。**コンセントの形状という規格(インターフェース)**にだけ依存しています。だから電力会社が火力から太陽光に変わってもドライヤーは動くし、同じコンセントに掃除機も挿せます。もしドライヤーが発電所の送電線に直接ハンダ付けされていたら(=具象クラスへの直接依存)、発電所の変更のたびにドライヤーを改造する羽目になります。

「**具体(発電所)ではなく、抽象(コンセント規格)に依存せよ**」— DIP の言っていることはこれだけです。テストでモックに差し替えられるのも、「テスト用の小型発電機」を同じコンセントに挿しているにすぎません。

SOLID 全体への向き合い方も添えておきます。5原則の暗唱には価値がありません。すべては「**変更が来たとき、修正箇所が局所で済むか?**」という1つの問いの、5つの言い換えです。迷ったら原則名ではなく、この問いに戻ってください。
