...
## 💡 コラム: コンウェイの法則 — システムは組織のコピーになる

1967年、メルヴィン・コンウェイという計算機科学者が、奇妙な法則を発表しました。「**システムを設計する組織は、その組織のコミュニケーション構造をそっくり写し取った構造の設計を生み出してしまう**」— 通称コンウェイの法則です。

有名な冗談型の言い換えがあります: 「**4つのチームでコンパイラを作らせると、4パスのコンパイラができあがる**」。チーム間の会話の境界線が、そのままシステムの部品の境界線になるのです。半分冗談に聞こえますが、その後の実証研究でも支持されており、50年以上生き残っている法則です。

これはアーキテクチャ設計に深い示唆を与えます。マイクロサービスが流行した本当の理由の一つは技術ではなく組織 — 「チームが互いに調整会議なしで独立にリリースできる構造が欲しい」でした。逆に3人のチームが10個のマイクロサービスを運用するのは、法則に逆らった構造で苦しみます。**アーキテクチャの議論は、実は組織とコミュニケーションの議論でもある** — この視点を持つと、設計判断の「なぜ」が急に読めるようになります。
