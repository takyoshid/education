...
VS Code の開発を率いるエリック・ガンマは、実はソフトウェア設計史の伝説的人物です。1994年の名著『デザインパターン』(通称 GoF 本 — Phase 7 で学びます)の著者の一人であり、テストフレームワーク JUnit をケント・ベックと共同開発し、Java の定番エディタ Eclipse の中核を設計した人物でもあります。

1990年代に設計理論の教科書を書いた人が、2020年代に世界で最も使われるエディタを作っている — ここには重要な示唆があります。**エディタもフレームワークも流行り廃りしますが、「良い設計とは何か」という基礎は数十年単位で通用する**のです。

VS Code 自体が設計の教材でもあります。本体は最小限に保ち、機能は拡張機能(プラグイン)として分離するアーキテクチャは、Phase 7 で学ぶ「開放閉鎖原則」の実践例そのもの。ショートカットを覚えるついでに、「なぜこの道具はこう作られているのか」という視点も持ってみてください。
